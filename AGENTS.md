...
- 開発元のアップデート更新対象は `Core/` 直下の開発元管理ファイル、`Core/App/`、`Core/Data/` 内の開発元管理データとすること。
- 開発元は `Core/` 直下の開発元管理ファイル、`Core/App/`、テーマ関連ソースコード、多言語化データを更新、改良、バグ修正できること。
- テーマ関連ソースコードは開発元管理とし、アップデート時に上書きされる前提とすること。
- ユーザーによるテーマ関連ソースコードの修正・カスタマイズを前提にしないこと。
- テーマ関連データはユーザーがカスタマイズ可能とし、アップデート時に上書き、削除、初期化しないこと。
- 多言語化データはCMSサーバ等の多言語化のための開発元管理データとし、アップデート時に上書きされる前提とすること。
- 認証情報はアップデート時に上書き、削除、初期化しないこと。
- 開発元は `Core/Config/`、`Modules/`、`Work/`、`Docs/`、コンテンツデータ、公開成果物、運用履歴に関与しないこと。
- 作業データを `Core/` 直下または `Core/Data/` に作成・維持しないこと。
